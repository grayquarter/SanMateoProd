...
try {
    ...
    aa.env.setValue("GQ_publicUserExists", result);
}
catch (err) {
    logDebug(err)
}
...
function addContactFromRef(paramCap, contactRefNum) {
    ...
    if(!result.getSuccess()) {
        logDebug("Error retrieving contact.");
        return false;
    }
    ...
    if(!addContact.getSuccess()) {
        logDebug("Error adding reference contact to record.");
        return false;
    }

    logDebug("Successfully added reference contact to record.");
    return true;
}