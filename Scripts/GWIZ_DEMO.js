...
try {
    var testSessionId;
    // testSessionId = "8cbbd0afa6f84719ad33b78182f53f87";
    if (testSessionId) aa.env.setValue("gqinsession_id", testSessionId);

    // Get the session object from the wizard
    ...
    // The record ID that was entered in the wizard
    var recordID = formData.recordID;

    // The variables we will (eventually) return to the wizard
    var recordType = "NA";
    var recordStatus = "NA";

    // Get the record type and status from the record ID
    var capResult = aa.cap.getCapID(recordID);
    if (capResult.getSuccess()) {
        var capId = capResult.getOutput();
        var cap = aa.cap.getCap(capId).getOutput();
        recordType = cap.getCapType().toString();
        recordStatus = cap.getCapStatus().toString();
    }

    // Set the return values for the wizard
    aa.env.setValue("recordType", recordType);
    aa.env.setValue("recordStatus", recordStatus);
}
catch (err) {
    logDebug(err)
}
