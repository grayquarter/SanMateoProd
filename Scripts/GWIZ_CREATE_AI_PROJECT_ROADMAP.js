...
try {
    ...
    for (var i in recordTypes) {
        ...
        var feeEstimateObj = getFeeTable(capId);
        ...
        var recordData = {
            ...
            "TABLE-payNow": feeEstimateObj.payNowTable,
            "TABLE-payLater": feeEstimateObj.payLaterTable
        };
...
function getFeeTable(capId) {
    ...
    var asaTotal = 0;
    var payNowTable = {
        "columns": ["Name", "Quantity", "Amount Due"],
        "rows": [],
        "totals": [] // Optional
    };
    var payNowFeesList = [];
    for (var i in feeItems) {
        ...
        if (feeItem.source == "ASA" || feeItem.source == "Configuration") {
            payNowTable.rows.push([feeItem.name, feeItem.quantity, formatCurrency(feeItem.due)]);
            asaTotal += parseInt(feeItem.due, 10);
            payNowFeesList.push(String(feeItem.name));
        }
    }
    logDebug(payNowFeesList.join(","));
    ...
    payNowTable.totals = ["Total", "", formatCurrency(asaTotal)];
    for (var i in feeItems) {
        var feeItem = feeItems[i];
        if (feeItem.source == "CRCA" && !exists(feeItem.name, payNowFeesList)) {
            payLaterTable.rows.push([feeItem.name, feeItem.quantity, formatCurrency(feeItem.due)]);
            crcaTotal += parseInt(feeItem.due, 10);
        }
    }
    payLaterTable.totals = ["Total", "", formatCurrency(crcaTotal)];

    var result = {
        "payNowTable": payNowTable,
        "payLaterTable": payLaterTable
    }
...
function formatCurrency(amount) {
    var number = Math.abs(amount).toFixed(2); // Ensure two decimal places
    var sign = amount < 0 ? '-' : ''; // Handle negative numbers
